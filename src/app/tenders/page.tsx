        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
          ...
          {!loading && !error && tenders.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">There are currently no tenders available. Check back later!</p>
            </div>
          )}
          ...
          {!loading && !error && tenders.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tenders.map((tender) => {
                ...
                return (
                  <div key={tender.id} className="bg-gradient-to-br from-white to-gray-50 rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-200">
                    <div className="flex items-start justify-between mb-4">
                      <h3 className="text-lg font-semibold text-black line-clamp-2">{parsed.title}</h3>
                      <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getStatusColor(parsed.status)}`}>
                        {parsed.status}
                      </span>
                    </div>
